...
import { BetParams, createBet, updateBet } from "./BetActions";
...
export function EditBetForm({ setOpen, bet }: EditBetFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      amountBet: Number(bet.amountBet) || 0,
      amountWon: Number(bet.amountWon) || 0,
      date: bet.date,
      status: bet.status,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const result = await updateBet(betData);
    setLoading(false);
    if (result) {
      setOpen(false);
      toast({
        title: "Bet Updated Successfully",
        description: "We've updated your bet for you.",
        duration: 2000,
      });
    } else {
